...
from reward_machines.ethical_reward_machine import Act_Deontology, Utilitarian
...
def make_env(env_id, env_type, args, mpi_rank=0, subrank=0, seed=None, reward_scale=1.0, gamestate=None, flatten_dict_observations=True, wrapper_kwargs=None, env_kwargs=None, logger_dir=None, initializer=None):
    ...
    if args.use_ActDeon:
        env = Act_Deontology(env)

    if args.use_Utilitarian:
        env = Utilitarian(env)
...
def common_arg_parser():
    ...
    # ERM-related arguments
    parser.add_argument("--use_ActDeon", help="Use Act-Deontology experience", action="store_true", default=False)
    parser.add_argument("--use_Utilitarian", help="Use Utilitarian experience", action="store_true", default=False)
    return parser
