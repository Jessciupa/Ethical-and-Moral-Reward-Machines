        ...
        for s1 in S:
            for u1 in U:
                ...
                for a in A:
                    ...
                    try:
                        u2, r, done, _ = rm.step(u1, l, None)  # Update here
                    except ValueError as e:
                        print("Error:", e)
                        print("Returned values:", rm.step(u1, l, None))
                        continue
                    if done:
                        q_values.append(r)
                    else:
                        q_values.append(r + gamma * V[(s2, u2)])
                v_new = max(q_values, default=0)  # Update here
        ...
        for u1 in U:
            ...
            for a in A:
                ...
                if done:
                    ...
                    q_values.append(r + gamma * V[(s2, u2)])
            a_i = max((x, i) for i, x in enumerate(q_values))[1]  # argmax over the q-values
            policy[(s1, u1)] = A[a_i]
