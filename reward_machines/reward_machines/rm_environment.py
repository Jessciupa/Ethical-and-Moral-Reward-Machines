...
class RewardMachineEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # update the RM state
        self.current_u_id, rm_rew, rm_done, _ = self.current_rm.step(self.current_u_id, true_props, self.crm_params, add_rs=False, env_done=env_done)
...
class RewardMachineWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if self.add_crm:
            ...
            _, rs_rm_rew, _,_ = rm.step(u_id, rs_true_props, rs_info, self.add_rs, rs_env_done)
    ...
    def _get_rm_experience(self, rm_id, rm, u_id, obs, action, next_obs, env_done, true_props, info):
        ...
        next_u_id, rm_rew, rm_done, _ = rm.step(u_id, true_props, info, self.add_rs, env_done)
        done = rm_done or env_done
...
class HierarchicalRMWrapper(gym.Wrapper):
    ...
    def _get_option_experience(self, option_id, obs, action, next_obs, env_done, true_props, info):
        ...
        results = rm.step(u1, true_props, info, self.add_rs, env_done)
        un, rm_rew,_,_ = results
    ...
    def get_experience(self):
        ...
        for option_id in range(self.num_options):
            ...
            step_results= rm.step(u1, true_props, info)

            un, _, _,_ = step_results
